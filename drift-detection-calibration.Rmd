...
get_current_proxy_data <- function(pollutant){
  ...
  while(request_start_time < as.character(Sys.time() - td_day)){  
    ...
    month_after_last_start <- as.character(ymd_hms(request_start_time) + td_30day + td_hour)
    if(is.na(month_after_last_start)){month_after_last_start <- paste(as.character(as.Date(request_start_time) + td_30day), '01:00:00')}
...
```{r}
combine_new_and_old_params <- function(new_params){
  old_params_path <- list.files('results', 'OZONE*', full.names = T, recursive = F)
  
  old_params <- read.csv(old_params_path)
  ...
  new_params <- full_join(aqy_metadata, new_params, by = 'ID') %>%
    na.omit() %>%
    mutate(start_date = '2021-07-08 11:00:00', end_date = '2199-12-31 23:59:59')
...
```{r}
new_params <- data.frame('AQY BB-633', 5, 4)
colnames(new_params) <- c('ID', 'O3.gain', 'O3.offset')

combine_new_and_old_params(new_params)
```
