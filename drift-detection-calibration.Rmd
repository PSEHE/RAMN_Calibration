...
aqy.proxy.data.PM25 <- inner_join(aqy.data.PM25, proxy.data.PM25, by = c('pollutant_proxy_site', 'timestamp_pacific')) %>%
  mutate(time_month = month(timestamp_pacific), time_day_of_month = as.numeric(str_sub(timestamp_pacific, 9, 10)), time_year = year(timestamp_pacific), 
         .after = 'timestamp_pacific')

mid.month.PM25 <- aqy.proxy.data.PM25  %>%
  filter(time_day_of_month >= 10 & time_day_of_month <= 19)
