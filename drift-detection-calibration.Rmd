...
filter_and_join_data <- function(aqy_data, proxy_data, start_time, td_add_to_start){
  ...
  joined_data <- inner_join(aqy_data, proxy_data, by = c('timestamp', 'proxy_site')) %>%
    filter(ID %in% c('AQY BB-633', 'AQY BB-643') & timestamp < '2021') %>%
...
check_for_five_days_of_flags <- function(summed_flags){
  ...
  aqy_with_five_days_of_flags <- filter(summed_flags, max_flag >= 24*5) %>%
    pull(ID)
  
  return(aqy_with_five_days_of_flags)
...
recalibrate_O3 <- function(joined_data_30day){
  ...
  O3.gain.squared <- var(joined_data_30day$proxy_rand) / var(joined_data_30day$O3)
  print(O3.gain.squared)
  
  O3.gain <- sqrt(O3.gain.squared)
  O3.offset <- mean(joined_data_30day$proxy_rand) - O3.gain*mean(joined_data_30day$O3)
  
  aqy_id <- 'AQY BB-633'
  
  print(paste('New gain:', O3.gain, '| New offset:', O3.offset))
  
  # Combined results
  O3_params <- as.data.frame(cbind(aqy_id, O3.gain, O3.offset))
  colnames(O3_params) <- c('ID', 'O3.gain', 'O3.offset')
...
combine_new_and_old_params <- function(new_params, start_time_rolling_72hr){
  ...
  combined_params <- rbind(old_params, new_params) %>%
    ...
    group_by(ID) %>%
    mutate(end_date = ifelse(end_date == '2199-12-31 23:59:59' & start_date != max(start_date), format_timestamp(max(start_date) - td_hour), end_date)) %>%
    unique()
...
reset_flags_after_calibration <- function(){
  ...
  read.csv('results/running_flags/running_flags_OZONE.csv') %>%
    mutate_at(.vars = c('ks', 'gain', 'offset', 'missing', 'extreme'), .funs = ~.x*0) %>%
