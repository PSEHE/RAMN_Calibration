...
find_starttime_proxy_request <- function(existing_proxy_data){
  ...
  ramn_start_time <- '2019-12-01 00:00:00'
  yesterday <- as.character(Sys.time() - td_day)
  right_now <- as.character(round(Sys.time(), 'hour'))
  ...
  next_timestamp_needed <- as.character(ymd_hms(last_timestamp_recorded) + td_hour)
...
build_proxy_request_url <- function(pollutant, start_request){
  ...
  month_after_start <- suppressWarnings(as.character(ymd_hms(start_request) + td_30_days))
  if(is.na(month_after_start)){month_after_start <- paste(as.character(as.Date(start_request) + td_30_days), '00:00:00')}
...
clean_new_proxy_data <- function(new_proxy_data){
   ...
   random_noise <- runif(nrow(new_proxy_data), .000001, .00001)
...
get_current_proxy_data <- function(pollutant){
  ...
  proxy_data <- get_existing_proxy_data(pollutant)

  request_start_time <- find_starttime_proxy_request(proxy_data)
  ...
  while(request_start_time < as.character(Sys.time() - td_day)){  
    ...
    month_after_last_start <- as.character(ymd_hms(request_start_time) + td_30_days + td_hour)
    if(is.na(month_after_last_start)){month_after_last_start <- paste(as.character(as.Date(request_start_time) + td_30_days), '01:00:00')}
    
    request_start_time <- min(as.character(Sys.time() - td_day), month_after_last_start)
  }
  
  updated_proxy_data <- process_and_write_proxy_data(pollutant, proxy_data)
...
filter_and_join_data <- function(aqy_data, proxy_data, start_time, td_add_to_start){
  ...
  joined_data <- inner_join(aqy_data, proxy_data, by = 'timestamp') %>%
    filter(proxy_site == 'San Pablo - Rumrill') %>%
    filter(ID == 'AQY BB-633' & timestamp < '2021') %>%
    filter(timestamp < as.character(ymd_hms(start_time) + td_add_to_start)) %>%
    na.omit()
...
```{r}
run_ks_test <- function(joined_data){
  
  proxy_values <- joined_data$proxy_rand
  aqy_values <- joined_data$O3_raw
  
  ks_results <- suppressWarnings(ks.test(proxy_values, aqy_values, exact = F))
    
  ks_p <- ks_results$p.value
  print(paste('KS Test P-val:', ks_p))
  
  ks_flag <- ifelse(ks_p <= 0.05, 1, 0)
...
```{r}
calculate_gain <- function(joined_data){
  
  proxy_values <- joined_data$proxy_rand
  aqy_values <- joined_data$O3_raw
  
  manual_gain <- sqrt(var(proxy_values)/var(aqy_values))
  print(paste('Manual gain:', manual_gain))
  
  gain_flag <- ifelse(manual_gain > 1.3 | manual_gain < .7, 1, 0)
...
```{r}
calculate_offset <- function(joined_data){
  
  proxy_values <- joined_data$proxy_rand
  aqy_values <- joined_data$O3_raw
  
  manual_offset <- sqrt(var(proxy_values)/var(aqy_values))
  print(paste('Manual offset:', manual_offset))
  
  offset_flag <- ifelse(manual_offset > 1.3 | manual_offset < .7, 1, 0)
...
detect_drift <- function(joined_data_72hr){
          ...
          ks_new <- run_ks_test(joined_data_72hr)
          gain_new <- calculate_gain(joined_data_72hr)
          offset_new <- calculate_offset(joined_data_72hr)
          missing_new <- ifelse(nrow(joined_data_72hr) <= 0.75*72, 1, 0)
          extreme_new <- 0
          # add test for extreme values relative to rest of data distribution
          
          new_flags <- as.data.frame(cbind(ID, ks_new, gain_new, offset_new, missing_new, extreme_new)) %>%
            mutate_at(.vars = c('ks_new', 'gain_new', 'offset_new', 'missing_new', 'extreme_new'), .funs = as.integer)
...
sum_old_and_new_flags <- function(pollutant, new_flags){
  ...
  summed_flags <- mutate_at(.tbl = joined_flags, .vars = cols_to_rmv_na, .funs = ~replace_na(.x, 0)) %>%
    transmute(ID = ID, 
              ...
              missing = sum_if_flagged(missing, missing_new),
              extreme = sum_if_flagged(extreme, extreme_new))
...
```{r}
check_for_five_days_of_flags <- function(summed_flags){
  
  largest_running_flag <- apply(X = summed_flags[2:ncol(summed_flags)], MARGIN = 1, FUN = max)
  
  summed_flags$max_flag <- largest_running_flag
  
  aqy_with_five_flags <- filter(summed_flags, max_flag >= 24*5) %>%
    pull(ID)
  
  return(aqy_with_five_flags)
...
```{r}
recalibrate_O3 <- function(joined_data_30day){
  
  O3.gain <- sqrt(var(joined_data_30day$proxy_rand) / var(joined_data_30day$O3_raw))
  O3.offset <- mean(joined_data_30day$proxy_rand) - O3.gain*mean(joined_data_30day$O3_raw)
...
```{r}
check_drift_calibrate_data <- function(aqy_data, proxy_data){
  
  start_time_rolling_72hr <- '2020-07-01 08:00:00'
  start_time_recalibration <- as.character(ymd_hms(start_time_rolling_72hr) - td_30day)
  
  joined_data_72hr <- filter_and_join_data(aqy_data, O3_proxy, '2020-07-01 08:00:00', td_72hr)
  joined_data_30day <- filter_and_join_data(aqy_data, O3_proxy, '2020-07-01 08:00:00', td_30day)
  ...
  aqys_with_five_flags <- check_for_five_days_of_flags(summed_flags)
  
  if(length(aqys_with_five_flags) < 1){return(print('No monitors require recalibration for this 72-hr period'))}
  
  new_params <- recalibrate_O3(joined_data_30day)
  
  return(new_params)
}

check_drift_calibrate_data(aqy_data, O3_proxy)
...
calibrate_monitors <- function(start_72, pollutant){
  ...
  aqy_data_720_raw <- read_aqy_data(pollutant, start_recal, end_recal)
  aqy_data_720 <- calibrate_with_old_params(pollutant, aqy_data_720_raw)
...
run_this_hour_O3 <- fix_midnight(as.character(ymd_hms('2020-06-03 00:00:00') + 60*60))
run_this_hour_NO2 <- fix_midnight(as.character(ymd_hms('2020-06-03 00:00:00') + 60*60))
