...
aqy_flags <- mutate(aqy_list, flag_KS_now = 0, flag_slope_now = 0, flag_offset_now = 0)
...
aqy_flags$flag_KS_now[1] <- ifelse(ks_results$p.value <= 0.05, 1, 0) #If p-value less than 0.05 add flag, else flag = 0

## Flag via slope drift
manual_slope <- sqrt(var_sp/var_aqy)
aqy_flags$flag_slope_now[1] <- ifelse(manual_slope > 1.3 | manual_slope < .7, 1, 0) # Assign flag if manual slope outside bounds, else flag = 0

## Flag via offset drift
manual_offset <- (mean_aqy-mean_sp)*sqrt(var_sp/var_aqy)
aqy_flags$flag_offset_now[1] <- ifelse(manual_offset > 5 | manual_offset < 5, 1, 0) # Assign flag if manual offset outside bounds, else flag = 0
...
running_flags <- read.csv('results/running_flags.csv') %>% inner_join(., aqy_flags, by = 'Monitor.ID') #Combine running flags with today's flags
...
```{r}
### RECALIBRATE IF FLAG REACHES FIVE
```
