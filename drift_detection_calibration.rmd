...
cal_params_full <- read.csv('results/cal_params_O3_original.csv', stringsAsFactors = F) %>% 
...
recalibrate_O3 <- function(start_72, end_72){
    ...
    ### DRIFT DETECTION
        ...
        for(i in 1:nrow(aqy_O3_id)){
            ...
            if(nrow(aqy_O3_select) <= 72*.75){
              todays_flags$ks_O3[i] <- -1
              print(paste('Did not perform drift detection for', aqy_ID, 'due to insufficient data. nrow = ', nrow(aqy_O3_select)))
              }
            
            else{
              
              # Filter to appropriate proxy
              proxy_stn <- unique(aqy_O3_select$proxy_site)
              ...
              aqy_O3_select <- aqy_O3_select %>% 
                rename('value'='O3_cal')
              
              ## Flag via ks test
              ks_results <- suppressWarnings(ks.test(proxy_O3$original_value, aqy_O3_select$value, exact = F))
              ...
              # Slope drift
              var_aqy <- var(aqy_O3_select$value, na.rm = T)
              ...
              # Offset drift
              mean_aqy <- mean(aqy_O3_select$value, na.rm = T)
              ...
              ## Print parameters
              print(paste(aqy_ID, 'from', start_72, 'to', end_72, '|| KS P-VALUE:', ks_p, '| MANUAL SLOPE:', manual_slope, '| MANUAL OFFSET:', manual_offset))
            }
          }
    ...
    if(length(needs_calibration) == 0){
      ...
      return(running_flags)
        ...
        # Add new parameters to list of old and write output
        ...
        param_new_O3 <- left_join(new_params, param_info, by = 'ID') %>%
          dplyr::select(ID, description, address, city, proxy_site, lat, lon, O3.slope, O3.offset, start_date, end_date)
        ...
        param_existing_O3 <- read.csv(paste0('results/',param_path_current), stringsAsFactors = F) %>%
          dplyr::select(ID, c(description, address, city, proxy_site, lat, lon, O3.slope, O3.offset, start_date, end_date)) 
        param_existing_O3$end_date <- ifelse(param_existing_O3$ID %in% param_new_O3$ID & end_date == '9999-12-31', as.character(as.Date(param_new_O3$start_date)-1), param_existing_O3$end_date)
        ...
        dplyr::select(running_flags, -c(max_flag, ks_O3, slope_O3, offset_O3)) %>%
          mutate(ks_O3_run = ifelse(ID %in% needs_calibration, 0, ks_O3_run),
                 slope_O3_run = ifelse(ID %in% needs_calibration, 0, slope_O3_run), 
...
end_dates <- seq(Sys.Date()-203, Sys.Date()-1, 1)
start_dates <- seq(Sys.Date()-205, Sys.Date()-3, 1)
