...
process_new_params <- function(pollutant, new_params_list, flag_detected){
  ...
  colnames(new_params) <- ifelse(pollutant == 'OZONE', c('ID', 'gain', 'offset'), c('ID', 'NO2.b0', 'NO2.b1', 'NO2.b2', 'NO2.b0_kl', 'NO2.b1_kl', 'NO2.b2_kl'))
...
combine_old_and_new <- function(pollutant, new_params){
  ...
  monitor_info <- dplyr::select(params_existing, c('ID', 'description', 'address', 'city', 'lat', 'lon'))
  params_current <- inner_join(monitor_info, new_params, by = 'ID')
...
```{r}
calibrate_monitors(as_date('2019-12-10'), 'OZONE')
```
