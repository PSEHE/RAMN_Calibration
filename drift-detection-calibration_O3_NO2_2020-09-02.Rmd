...
```{r}
format_dates <- function(desired_time){date_and_time <- suppressWarnings((ifelse(!is.na(ymd_hms(desired_time)), as.character(desired_time), paste(desired_time, '00:00:00'))))
...
Note on below - may want to modify so it just starts from the first timestamp after the last one - NO2 totally blacked out on 02-29-20 so it goes all the way back every time
```{r}
find_starttime_proxy_request <- function(existing_proxy_data){
  ...
  right_now <- as.character(Sys.time())
  ...
  timestamps_needed <- as.character(seq(from = ymd_hms(ramn_start_time), to = ymd_hms(yesterday), by = 60*60))

  timestamps_not_existing <- subset(timestamps_needed, !(timestamps_needed %in% timestamps_existing))
  
  if(length(timestamps_not_existing) == 0){return(right_now)}
          
  first_timestamp_needed <- min(timestamps_not_existing)
  
  print(paste('Requesting data starting with', first_timestamp_needed))
  
  return(first_timestamp_needed)
...
build_proxy_request_url <- function(pollutant, start_request){
  ...
  month_after_start <- as.character(ymd_hms(start_request) + 60*60*24*30)
  yesterday <- as.character(Sys.time() - 60*60*24)
...
get_current_proxy_data <- function(pollutant){
  ...
  while(request_start_time < as.character(Sys.time() - 60*60*24)){  
    ...
    request_start_time <- as.character(ymd_hms(request_start_time) + 60*60*24*30 + 60*60)
  }
...
```{r}
ozone_proxy <- get_current_proxy_data('OZONE')
...
```{r}
read_aqy_data <- function(pollutant, first_hr_existing_aqy, last_hr_existing_aqy){
  ...
  aqy_data_directory <- 'C:/Users/18313/Desktop/richmond_air/downloader/results/concatenated_data'  #### CHANGE PATH AS APPROPRIATE FOR ENVIRONMENT
  ...
  print(paste('Reading most current AQY data found at:', aqy_data_path_newest))
  
  aqy_data_filtered <- read.csv(aqy_data_path_newest, stringsAsFactors = F) %>%
    rename('timestamp'='Time') %>%
    mutate(timestamp = as.character(ymd_hms(timestamp) + 8*60*60)) %>% #Convert to UTC
    filter(timestamp >= first_hr_existing_aqy & timestamp <= last_hr_existing_aqy)
  
  return(aqy_data_filtered)
}
```

```{r}
calibrate_with_old_params <- function(pollutant, raw_aqy_data){
  ...
  print(paste('Calibrating O3 values with parameters at path', cal_files_O3_newest))
  
  cal_file_O3 <- filter(read.csv(cal_files_O3_newest, stringsAsFactors = F), !is.na(lat))
  aqy_data_O3 <- inner_join(raw_aqy_data, cal_file_O3, by = 'ID') %>% 
  ...
  if(pollutant == 'NO2'){
    ...
    print(paste('Calibrating NO2 values with parameters at path', cal_files_NO2_newest))
